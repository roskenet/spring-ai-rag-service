...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Usage Analytics API - Auth header present:', !!authHeader);

    const { searchParams } = new URL(request.url);
    const queryString = searchParams.toString();

    // Build backend URL using centralized configuration
    const backendUrl = buildBackendUrl('/analytics/usage', queryString);
    ...
    if (authHeader) {
      ...
      console.log('Usage Analytics API - Forwarding auth to backend');
    } else {
      console.log('Usage Analytics API - No auth header to forward (proceeding anyway)');
    }
    ...
    console.error('Usage analytics proxy error:', error);
    return NextResponse.json(
      {
