...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Analytics Dashboard API - Auth header present:', !!authHeader);

    // Build backend URL using centralized configuration
    const backendUrl = buildBackendUrl('/analytics/dashboard');
    ...
    if (authHeader) {
      ...
      console.log('Analytics Dashboard API - Forwarding auth to backend');
    } else {
      console.log('Analytics Dashboard API - No auth header to forward (proceeding anyway)');
    }
