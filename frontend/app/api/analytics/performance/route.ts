...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Performance Analytics API - Auth header present:', !!authHeader);

    const { searchParams } = new URL(request.url);
    const queryString = searchParams.toString();

    // Build backend URL using centralized configuration
    const backendUrl = buildBackendUrl('/analytics/performance', queryString);
    ...
    if (authHeader) {
      ...
      console.log('Performance Analytics API - Forwarding auth to backend');
    } else {
      console.log('Performance Analytics API - No auth header to forward (proceeding anyway)');
    }
    ...
    console.error('Performance analytics proxy error:', error);
    return NextResponse.json(
      {
